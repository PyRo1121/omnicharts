...
export async function applyStreamOnline(env: Env, event: StreamOnlineEvent): Promise<void> {
	...
	const row = await db.prepare(
		`SELECT id FROM channels WHERE platform_id = ? AND platform_channel_id = ?`
	)
...
export async function applyStreamOffline(
	env: Env,
	...
	const db = requireDb(env);
	const endedAt =
		(typeof event.ended_at === 'string' && event.ended_at.length > 0
			? event.ended_at
			: undefined) ??
		opts?.endedAt ??
		nowIso();
	...
	await db.prepare(
		`UPDATE channels SET last_seen_at = ? WHERE id = ?`
	)
		.bind(endedAt, row.id)
		.run();

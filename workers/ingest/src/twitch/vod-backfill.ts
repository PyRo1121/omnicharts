import {
	VOD_BACKFILL_STALE_DAYS,
	VOD_BACKFILL_VIDEOS_FIRST,
	vodBackfillMaxChannelsFromEnv
} from './config';
...
async function resolveBackfillChannels(
	db: D1Database,
	opts: VodBackfillOptions,
	maxChannels: number
	...
	if (opts.platformChannelIds?.length) {
		...
		return listChannelsForVodBackfillByPlatformIds(db, ids).then((rows) =>
			rows.slice(0, maxChannels)
		);
	}
...
function filterVideosForChannel(
	...
	return videos.filter((video) =>
		isVideoWithinRetention(video.published_at || video.created_at, retentionDays, nowMs)
	);
}
...
export async function runTwitchVodBackfill(
	env: Env,
	opts: VodBackfillOptions = {}
): Promise<VodBackfillStats> {
	...
	for (const channel of channels) {
		const { videos, pages } = await fetchAllArchiveVideosForUser(
			client,
		...
		const rows = inWindow.map((video) =>
			helixVideoToVodSessionRow(
				channel.id,
				video,
				vodSessionTimes(video, nowMs)
			)
		);

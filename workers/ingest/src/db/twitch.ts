...
import { batchRecordLiveSamples, batchUpsertChannelsFromStreams, batchUpsertGameCategories } from './twitch-live-batch';
import { runD1Batches } from './d1-batch';

export { batchRecordLiveSamples, batchUpsertChannelsFromStreams, batchUpsertGameCategories } from './twitch-live-batch';
...
export async function upsertGameCategory(db: D1Database, game: Pick<HelixGame, 'id' | 'name'>): Promise<string> {
...
export async function listRecentlyTrackedPlatformIds(db: D1Database, withinHours: number, limit: number): Promise<string[]> {
...
export async function listChannelIdsToPoll(db: D1Database, limit: number): Promise<string[]> {
	const { results } = await db
		.prepare(
			`SELECT platform_channel_id FROM channels
       WHERE ${TRACKED_TWITCH_CHANNEL_SQL}
...
export async function listPlatformIdsForRollupDate(db: D1Database, rollupDate: string, limit: number): Promise<string[]> {
...
export async function listPlatformIdsForProfileEnrichment(db: D1Database, limit: number, staleHours: number): Promise<string[]> {
...
export async function applyChannelProfileEnrichment(db: D1Database, row: ChannelProfileEnrichmentRow): Promise<void> {
	await prepareChannelProfileUpdate(db, row, nowIso()).run();
}

/** Batch profile UPDATEs (≤50 per D1 `batch()`). */
export async function batchApplyChannelProfileEnrichment(db: D1Database, rows: ChannelProfileEnrichmentRow[]): Promise<void> {

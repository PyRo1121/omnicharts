...
export async function fetchFollowerCountsByChannelId(
	db: D1Database,
	channelIds: string[]
	...
	for (const batch of chunkArray(channelIds, D1_BATCH_MAX_STATEMENTS)) {
		...
		const { results } = await db
			.prepare(
				`SELECT id, follower_count FROM channels WHERE id IN (${placeholders})`
			)
			.bind(...batch)
...
export async function fetchPriorFollowerSnapshots(
	db: D1Database,
	channelIds: string[]
): Promise<Map<string, number | null>> {
	const map = new Map<string, number | null>();
...
export async function storeFollowerSnapshots(
	db: D1Database,

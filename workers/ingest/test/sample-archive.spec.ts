...
describe('sampleArchiveObjectKey', () => {
	it('uses hive path with platform and UTC day', () => {
		expect(sampleArchiveObjectKey(row, 'test-part')).toBe(
			'samples/year=2026/month=06/day=03/platform=twitch/part-test-part.ndjson'
		);
	});
});
...
describe('shouldArchiveSampleBatch', () => {
	it('blocks when disabled, missing bucket, or below threshold', () => {
		...
		expect(shouldArchiveSampleBatch({ SAMPLE_ARCHIVE_ENABLED: '0', SAMPLES: {} } as Env, 100)).toBe(
			'disabled'
		);
		expect(shouldArchiveSampleBatch({ SAMPLE_ARCHIVE_ENABLED: '1' } as Env, 100)).toBe('no_bucket');
		expect(
			shouldArchiveSampleBatch(
				{ SAMPLE_ARCHIVE_ENABLED: '1', SAMPLES: {}, SAMPLE_ARCHIVE_MIN_ROWS: '10' } as Env,
				9
			)
...
describe('archiveSampleBatch', () => {
	...
	it('no-ops when disabled', async () => {
		const put = vi.fn();
		expect(
			await archiveSampleBatch(
				{ SAMPLE_ARCHIVE_ENABLED: '0', SAMPLES: { put } } as unknown as Env,
				rows(20)
			)
	...
	it('writes NDJSON to R2 when enabled and at threshold', async () => {
		...
		expect(result.key).toMatch(
			/^samples\/year=2026\/month=06\/day=03\/platform=twitch\/part-.+\.ndjson$/
		);

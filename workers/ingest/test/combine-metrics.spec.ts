...
function sessionPoints(
	baseMs: number,
	session: { samples: { offsetMinutes: number; viewerCount: number }[] }
): ViewerSamplePoint[] {
...
describe('combineSessionMetrics', () => {
	...
	it('multi_stream_day: 2 streams, total HW 80', () => {
		const sessions = multi.sessions.map((s) =>
			aggregateSessionSamples(sessionPoints(baseMs, s), multi.defaultIntervalMinutes)
		);

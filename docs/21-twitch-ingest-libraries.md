...
Each `TwitchHelixClient` uses `phase_budget` when spawned by a **parallel** queue consumer (`poll_twitch_sweep` runs sweep **and** game pass inline; `poll_twitch_reconcile` is the second consumer). **Sequential** inline coverage (`runTwitchCoverageCycle`, legacy `poll_platform`) shares one client at `helixSafePointsPerMinuteFromEnv` and one `seenUserIds` set across sweep + game pass. Dynamic governors in [`rate-limit.ts`](../workers/ingest/src/twitch/rate-limit.ts) cap sweep pages and game-pass slices when remaining points are low ([22-ingest-free-tier-tuning](./22-ingest-free-tier-tuning.md)).

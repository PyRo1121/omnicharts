...
export async function firstRankedSlug(
	...
	const res = await fetch(
		`${INGEST_URL}/v1/rankings/${kind}?platform=${platform}&period=7d&limit=5`,
		{ signal: AbortSignal.timeout(5000) }
	);
...
export async function channelExistsOnPlatform(slug: string, platform: string): Promise<boolean> {
	const res = await fetch(
		`${INGEST_URL}/v1/channels/${encodeURIComponent(slug)}?platform=${encodeURIComponent(platform)}&period=7d`,
		{ signal: AbortSignal.timeout(5000) }
	);
	return res.ok;
}
...
export async function findKickOnlyChannelSlug(): Promise<string | null> {
	const res = await fetch(
		`${INGEST_URL}/v1/rankings/channels?platform=kick&period=7d&limit=10`,
		{ signal: AbortSignal.timeout(5000) }
	);
	if (!res.ok) return null;
	const body = (await res.json()) as { items?: RankedItem[] };
	for (const item of body.items ?? []) {
		...
		if (!slug) continue;
		const [onKick, onTwitch] = await Promise.all([
			channelExistsOnPlatform(slug, 'kick'),
			channelExistsOnPlatform(slug, 'twitch')
		]);

...
describe('parseCompareChannelsQuery', () => {
	...
	test('rejects invalid platform', () => {
		const url = new URL('http://localhost/v1/compare/channels?a=ninja&b=shroud&platform=invalid&period=7d');
	...
	test('accepts 30d and 90d compare periods', () => {
		const url = new URL('http://localhost/v1/compare/channels?a=ninja&b=shroud&platform=kick&period=90d');
		expect(parseCompareChannelsQuery(url)).toEqual({
			ok: true,
			platform: 'kick',
			period: '90d',
	...
	test('rejects invalid compare period', () => {
		const url = new URL('http://localhost/v1/compare/channels?a=ninja&b=shroud&platform=twitch&period=24h');
		expect(parseCompareChannelsQuery(url)).toEqual({ ok: false, error: 'invalid_period' });
	});

	test('trims slug whitespace', () => {
		const url = new URL('http://localhost/v1/compare/channels?a=%20ninja%20&b=shroud%20&platform=twitch');
		expect(parseCompareChannelsQuery(url)).toEqual({
			ok: true,
			platform: 'twitch',
			period: '7d',
...
describe('buildCompareChannelsResponse', () => {
	...
	test('marks missing channel as not found', async () => {
		const body = await buildCompareChannelsResponse(mockCompareDb({ missingSlug: 'missing' }), {
			platform: 'twitch',
			period: '7d',
			a: 'ninja',
			b: 'missing',

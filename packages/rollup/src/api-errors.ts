...
function jsonApiErrorResponse<E extends string>(
	code: E,
	message: string,
...
export function rankingsChannelsQueryErrorResponse(
	error: RankingsChannelsHttpQueryError,
	opts?: ErrorResponseOpts
): Response {
	return jsonApiErrorResponse(error, RANKINGS_CHANNELS_MESSAGES[error], opts);
}

export function rankingsGamesQueryErrorResponse(
	error: RankingsGamesHttpQueryError,
	opts?: ErrorResponseOpts
): Response {
	return jsonApiErrorResponse(error, RANKINGS_GAMES_MESSAGES[error], opts);
}

export function searchQueryErrorResponse(
	error: SearchHttpQueryError,
	opts?: ErrorResponseOpts
): Response {
	return jsonApiErrorResponse(error, SEARCH_MESSAGES[error], opts);
}

export function compareQueryErrorResponse(
	error: CompareHttpQueryError,
	opts?: ErrorResponseOpts
): Response {
	return jsonApiErrorResponse(error, COMPARE_MESSAGES[error], opts);
}

export function channelDetailQueryErrorResponse(
	error: ChannelDetailHttpQueryError,
	opts?: ErrorResponseOpts
): Response {
